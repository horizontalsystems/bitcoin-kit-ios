...
class InitialSyncer {
    ...
    init(realmFactory: IRealmFactory, hdWallet: IHDWallet, stateManager: IStateManager, apiSyncer: IApiSyncer, addressManager: IAddressManager, addressConverter: IAddressConverter, factory: IFactory, peerGroup: IPeerGroup, network: INetwork, scheduler: ImmediateSchedulerType = ConcurrentDispatchQueueScheduler(qos: .background)) {
    ...
    private func fetchFromApi(external: Bool, maxHeight: Int, lastUsedKeyIndex: Int = -1, keys: [PublicKey] = [], responses: [BlockResponse] = []) throws -> Observable<([PublicKey], [BlockResponse])> {
        ...
        return apiSyncer.getBlockHashes(publicKey: newKey)
